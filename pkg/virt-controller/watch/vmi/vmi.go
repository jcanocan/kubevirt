...
func (c *Controller) sync(vmi *virtv1.VirtualMachineInstance, pod *k8sv1.Pod, dataVolumes []*cdiv1.DataVolume) common.SyncError {
	...
	if !controller.PodExists(pod) {
		...
		if downwardmetrics.IsDownwardMetricsConfigurationInvalid(c.clusterConfig, &vmi.Spec) {
			c.recorder.Eventf(vmi, k8sv1.EventTypeWarning, controller.FeatureNotEnabled, downwardmetrics.NotEnabledError.Error())
			return common.NewSyncError(downwardmetrics.NotEnabledError, controller.FeatureNotEnabled)
		}
