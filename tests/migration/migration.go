...
var _ = SIGMigrationDescribe("VM Live Migration", decorators.RequiresTwoSchedulableNodes, func() {
	...
	Context("Virtiofs", decorators.VirtioFS, func() {
		DescribeTable("should migrate with a shared", func(createConfigVolume func() (string, string, string, []libvmi.Option)) {

			configVolumeName, expectedOutput, testCommand, configVolumeOption := createConfigVolume()
			...
			for _, opt := range configVolumeOption {
				opt(vmi)
			}
			...
			Entry("ConfigMap", func() (string, string, string, []libvmi.Option) {
				...
				return configMapName,
					"value1value2value3",
					"cat /mnt/option1 /mnt/option2 /mnt/option3\n",
					[]libvmi.Option{libvmi.WithConfigMapFs(configMapName, configMapName)}
			}),
			Entry("Secret", func() (string, string, string, []libvmi.Option) {
				secretName := "secret-" + uuid.NewString()[:6]
				secret := libsecret.New(secretName, libsecret.DataString{"user": "admin", "password": "redhat"})
				secret, err := kubevirt.Client().CoreV1().Secrets(testsuite.GetTestNamespace(secret)).Create(context.Background(), secret, metav1.CreateOptions{})
				Expect(err).ToNot(HaveOccurred())

				return secretName,
					"adminredhat",
					"cat /mnt/user /mnt/password\n",
					[]libvmi.Option{libvmi.WithSecretFs(secretName, secretName)}
			}),
			Entry("DownwardAPI", func() (string, string, string, []libvmi.Option) {
				downwardAPIName := "downwardapi-" + uuid.NewString()[:6]
				testLabelKey := "kubevirt.io.testdownwardapi"
				testLabelVal := "downwardAPIValue"

				return downwardAPIName,
					testLabelKey + "=" + "\"" + testLabelVal + "\"",
					"grep --color=never " + testLabelKey + " /mnt/labels\n",
					[]libvmi.Option{
						libvmi.WithDownwardAPIFs(downwardAPIName),
						libvmi.WithLabel(testLabelKey, testLabelVal),
					}
			}),
			Entry("ServiceAccount", func() (string, string, string, []libvmi.Option) {
				serviceAccountName := "serviceaccount-" + uuid.NewString()[:6]

				return serviceAccountName,
					testsuite.GetTestNamespace(nil),
					"cat /mnt/namespace\n",
					[]libvmi.Option{libvmi.WithServiceAccountFs("default", serviceAccountName)}
			}),
		)

		It("should migrate with a shared PVC", func() {
			_, foundSC := libstorage.GetRWXFileSystemStorageClass()

			if !foundSC {
				Skip("Skip test when Filesystem RWX storage is not present")
			}

			pvcName := "pvc-" + uuid.NewString()[:6]

			libstorage.CreateRWXFSPVC(pvcName, testsuite.GetTestNamespace(nil), "1Gi")
			...
			By("Checking the PVC is still mounted and testing file is accessible")
			Expect(console.SafeExpectBatch(vmi, []expect.Batcher{
